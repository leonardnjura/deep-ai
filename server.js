...
async function getMarkets(req, res, searchTerm, searchCol) {
	...
	try {
		...
		if (result.rows.length == 0) {
			return res.json({data: result.rows,msg: "no results matching your query", total_results: result.rows.length, code: 0});
		} else {
...
async function getCitizens(req, res, searchTerm, searchCol) {
	...
	var searchableFields = [ 'client_id', 'client_name', 'email', 'mobile_number', 'telephone_number', 'pin_number', 'created_date', 'modified_date' ];
